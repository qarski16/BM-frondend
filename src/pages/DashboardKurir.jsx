...
const DashboardKurir = () => {
  ...
  const fetchProfilDanStatusKurir = async () => {
    ...
    try {
      ...
      if (response.data && response.data.statusOnline) {
        ...
        return response.data.statusOnline;
      }
    } catch (err) {
  ...
  const handleUpdateStatusPesanan = async (pesananId, statusBaru) => {
    try {
      const token = localStorage.getItem('token');
      if (!token) { alert("Sesi Anda telah berakhir, silakan login kembali."); return; }
  ...
  const renderMainContent = () => {
    switch (activeMenu) {
                      ...
                      return (
                        <div key={pesananAktif._id || index} style={{ ...tableTugasWrapper, marginTop: index === 0 ? '0px' : '20px' }}>
                          ...
                          <div style={stepperContainer}>
                            <button onClick={() => handleUpdateStatusPesanan(pesananAktif._id, 'Ambil Barang')} disabled={currentStatus !== 'proses' && currentStatus !== 'pending'} style={(currentStatus === 'proses' || currentStatus === 'pending') ? stepActiveStyle : stepDisabledStyle}>Mulai Tugas</button>
                            <button onClick={() => handleUpdateStatusPesanan(pesananAktif._id, 'Dalam Perjalanan')} disabled={currentStatus !== 'ambil barang' && currentStatus !== 'on progress'} style={(currentStatus === 'ambil barang' || currentStatus === 'on progress') ? stepActiveStyle : stepDisabledStyle}>Ambil Barang</button>
                            <button onClick={() => handleUpdateStatusPesanan(pesananAktif._id, 'Sampai Tujuan')} disabled={currentStatus !== 'dalam perjalanan' && currentStatus !== 'shipping'} style={(currentStatus === 'dalam perjalanan' || currentStatus === 'shipping') ? stepActiveStyle : stepDisabledStyle}>Dalam Perjalanan</button>
                            <button onClick={() => handleUpdateStatusPesanan(pesananAktif._id, 'Selesai')} disabled={currentStatus !== 'sampai tujuan' && currentStatus !== 'out for delivery'} style={(currentStatus === 'sampai tujuan' || currentStatus === 'out for delivery') ? stepActiveStyle : stepDisabledStyle}>Sampai Tujuan</button>
                          </div>
                        </div>
                      );
                    })}
      ...
      case 'riwayat': return <RiwayatKurir />;
      case 'profil': return <ProfilKurir kurirId={kurirId} namaKurir={namaKurir} setNamaKurir={setNamaKurir} />;
      default: return null;
    }
  };
  ...
  return (
    <div style={containerStyle}>
      <aside style={sidebarStyle}>
        <div style={logoWrapper}><img src={logoBm} alt="Logo BM Kurir" style={logoImageStyle} /></div>
        <nav style={navStyle}>
          <button onClick={() => setActiveMenu('dashboard')} style={activeMenu === 'dashboard' ? navLinkActive : navLink}>Dashboard</button>
          <button onClick={() => setActiveMenu('riwayat')} style={activeMenu === 'riwayat' ? navLinkActive : navLink}>Riwayat</button>
          <button onClick={handleLogout} style={btnLogoutStyle}>Logout</button>
        </nav>
        <div onClick={() => setActiveMenu('profil')} style={{ ...profileFooter, backgroundColor: activeMenu === 'profil' ? '#22c55e' : '#14532d', cursor: 'pointer' }}>
      ...
      <div style={contentWrapperStyle}>{renderMainContent()}</div>
    </div>
  );
};
...
const profileFooter = { padding: '20px', borderTop: '1px solid #166534', display: 'flex', alignItems: 'center', gap: '12px' };
const avatarStyle = { width: '36px', height: '36px', borderRadius: '50%', backgroundColor: '#4b5563', display: 'flex', justifyContent: 'center', alignItems: 'center' };
...
const btnStatusStyle = { padding: '6px 14px', borderRadius: '20px', fontSize: '12px', fontWeight: '700', cursor: 'pointer' };
const tableTugasWrapper = { backgroundColor: 'white', borderRadius: '8px', border: '1px solid #cbd5e1', maxWidth: '600px', margin: '30px auto' };
const tableHeaderTitle = { backgroundColor: '#f8fafc', padding: '12px 20px', fontWeight: 'bold' };
const rowInfo = { display: 'flex', padding: '12px 20px', borderBottom: '1px solid #f1f5f9' };
const labelInfo = { width: '120px', color: '#64748b', fontSize: '12px' };
const valueInfo = { flex: 1, margin: 0, fontSize: '14px' };
const badgeStatusProses = { backgroundColor: '#fef3c7', color: '#d97706', padding: '6px 20px', borderRadius: '20px' };
const stepperContainer = { display: 'flex', justifyContent: 'center', gap: '10px', padding: '20px', backgroundColor: '#f8fafc' };
const stepActiveStyle = { backgroundColor: '#15803d', color: 'white', padding: '8px 15px', border: 'none', borderRadius: '4px', cursor: 'pointer' };
const stepDisabledStyle = { backgroundColor: '#cbd5e1', color: '#64748b', padding: '8px 15px', border: 'none', borderRadius: '4px', cursor: 'not-allowed' };
const emptyStateBox = { display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '50px 20px', border: '1px solid #cbd5e1', borderRadius: '8px', backgroundColor: 'white' };
const iconContainer = { width: '80px', height: '80px', backgroundColor: '#f3f4f6', borderRadius: '50%', display: 'flex', justifyContent: 'center', alignItems: 'center' };
const btnCekUlang = { marginTop: '20px', padding: '8px 16px', cursor: 'pointer' };
